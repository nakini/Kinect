function [Irec] = rect_uv_XYZ(I,R,f,c,k,alpha,KK_new)
...
% Add distortion:
xd = apply_distortion_uv_XYZ(x,k);
...
Irec(ind_new) = a1 .* double(I(ind_lu)) + a2 .* double(I(ind_ru)) + ...
    a3 .* double(I(ind_ld)) + a4 .* double(I(ind_rd));
