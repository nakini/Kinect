...
% Extrinsic parameters of the depth camera. These values are obtained from 
% Herrera's TPAMI 2012 paper.
fx_d = 579.83;      % Focal length in X and Y
fy_d = 586.73;
cx_d = 321.55;      % Principle point in X and Y
cy_d = 235.01;

% Find all the values which are in between .3 to 3 meters. We could extend the
% upper limit till 3.5 meters, as this is the upper limit of the kinect.
...
% Kinect is coordinate frame is different from the regular world frame. So first 
% transform all the point from the kinect frame to regular world frame.
Xw = Yd;
Yw = Zd;
Zw = -Xd;
