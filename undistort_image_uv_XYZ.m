function I2 = undistort_image_uv_XYZ(I, calibParams)
% This function takes a distorted image and the calibration parameters and then
% undistorts the same.
%
% INPUT(s):
%   I := MxN image matrix cotaining distorted pixels
%   calibParams := Mat file containing the calibratio parameters.
% OUPUT(s):
%   imgUndistorted := MxN matrix cotaining undistorted pixels

% Load the calibraio parameters and check whether all required paramters 
% available or not.
load(calibParams);
...
%     
    ...
    [I2] = rect_uv_XYZ(I,eye(3),fc,cc,kc,alpha_c,KK_new);
